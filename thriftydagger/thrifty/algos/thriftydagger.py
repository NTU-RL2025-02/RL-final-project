...
# =========================
# Helper functions for BC
# =========================


def _compute_loss_pi(ac, data, i):
    o, a = data["obs"], data["act"]
    a_pred = ac.pis[i](o)
    return torch.mean(torch.sum((a - a_pred) ** 2, dim=1))


def _update_pi(ac, optimizer, data, i):
    optimizer.zero_grad()
    loss_pi = _compute_loss_pi(ac, data, i)
    loss_pi.backward()
    optimizer.step()
    return loss_pi.item()


def _initial_bc_training(
    ac,
    replay_buffer,
    held_out_data,
    bc_epochs,
    ...
    """
    初始階段：用 offline BC 資料訓練 ensemble，每個 net 用 bootstrap 的 buffer。
    同時在 held_out_data 上計算 validation loss。
    """
    pi_optimizers = [Adam(ac.pis[i].parameters(), lr=1e-3) for i in range(ac.num_nets)]
    ...
    for i in range(ac.num_nets):
        ...
        for _ in range(bc_epochs):
            ...
            for _ in range(grad_steps):
                ...
                loss_pi_vals.append(_update_pi(ac, pi_optimizers[i], batch, i))
    ...
    return ac, pi_optimizers


def _retrain_bc_each_iter(
    ac,
    actor_critic,
    env,
    device,
    ...
    """
    每個 iter 結束後，重新初始化 ac，並用目前 replay_buffer 資料再訓練一次。
    注意 loss_pi 會被回傳，方便外面做 logging。
    """
    ac = actor_critic(
    ...
    for i in range(ac.num_nets):
        ...
        # retrain 次數會隨著 iter 增加
        for _ in range(grad_steps * (bc_epochs + t)):
            batch = tmp_buffer.sample_batch(batch_size)
            loss_pi_vals.append(_update_pi(ac, pi_optimizers[i], batch, i))

    return ac, pi_optimizers, loss_pi_vals
...
def _estimate_initial_thresholds(ac, replay_buffer, held_out_data, target_rate):
    """
    使用 offline data + held-out data 估計：
      - switch2robot_thresh: discrepancy 平均
      - switch2human_thresh: variance 的高 quantile
    """
    ...
    for i in range(len(held_out_data["obs"])):
        o = held_out_data["obs"][i]
        heldout_estimates.append(ac.variance(o))
...
def _update_thresholds_online(
    estimates, estimates2, target_rate, switch2human_thresh2, switch2robot_thresh2
):
    """
    每個 iter online 收集一批 estimates / estimates2 後，
    重新根據分位數更新 Risk-based threshold。
    """
    if len(estimates) <= 25:
        return switch2human_thresh2, switch2robot_thresh2

    target_idx = int((1 - target_rate) * len(estimates))
    # safety 越大越安全，因此 switch2human_thresh2 用 estimates2 由大到小排序
    new_switch2human_thresh2 = sorted(estimates2, reverse=True)[target_idx]
    # 取中位數當作往回切的 threshold
    new_switch2robot_thresh2 = sorted(estimates2)[int(0.5 * len(estimates))]

    return new_switch2human_thresh2, new_switch2robot_thresh2


# =========================
# Helper for Q-risk training
# =========================


def _train_qrisk_step(
    ac,
    ac_targ,
    qbuffer,
    gamma,
    ...
    for _ in range(bc_epochs):
        for _ in range(grad_steps * 5):
            ...
            loss_q_vals.append(loss_q.item())
            timer += 1
...
def thrifty(
    ...
    # Initial BC training
    # =====================
    ac, pi_optimizers = _initial_bc_training(
        ac=ac,
    ...
    # Initial thresholds
    # =====================
    switch2robot_thresh, switch2human_thresh = _estimate_initial_thresholds(
        ac, replay_buffer, held_out_data, target_rate
    )
    switch2human_thresh2 = 0.48  # initial guess
    switch2robot_thresh2 = 0.495

    torch.cuda.empty_cache()

    # held-out data 之後也回填到 replay buffer
    ...
    # =====================
    online_burden = 0
    num_switch_to_human = 0
    num_switch_to_human2 = 0
    ...
    for t in range(iters + 1):
        ...
        if t == 0:
            i = obs_per_iter  # iter 0 不收集新資料，留給 Q 用
        ...
        while i < obs_per_iter:
            ...
            # online 收集到足夠 estimates 後更新 risk-based threshold
            if len(estimates) > 25:
                new_eps_H, new_eps_R = _update_thresholds_online(
                    estimates,
                    estimates2,
                    target_rate,
                ...
                switch2human_thresh2 = new_eps_H
                switch2robot_thresh2 = new_eps_R
                print(
        ...
        # =====================
        if t > 0:
            ac, pi_optimizers, loss_pi_vals = _retrain_bc_each_iter(
                ac=ac,
        ...
        avg_loss_pi = sum(loss_pi_vals) / len(loss_pi_vals) if loss_pi_vals else 0.0
        avg_loss_q = sum(loss_q_vals) / len(loss_q_vals) if loss_q_vals else 0.0
