...
class HardcodedPolicy:
    ...
    def act(self, o):
        ...
        if (
            not self.env.gripper_closed
            and np.linalg.norm(grasp_point[:2] - eef_pos[:2]) > 0.005
        ):
        ...
        if (
            not self.env.gripper_closed
            and abs(rel_quat[0] + 1) > 0.01
            and abs(rel_quat[1] + 1) > 0.01
        ):
        ...
        if (
            np.linalg.norm(cylinder_pos[:2] - obj_pos[:2]) > 0.005
            and abs(cylinder_pos[2] - eef_pos[2]) > 0.01
        ):
