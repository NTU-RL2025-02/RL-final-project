...
def generate_offline_data(
    ...
    while i < num_episodes:
        ...
        if robosuite:
            if (
                total_ret > 0
            ):  # only count successful episodes for offline data collection
                i += 1
                obs.extend(curr_obs)
                act.extend(curr_act)
            env.close()
...
def lazy(
    ...
    replay_buffer.fill_buffer(
        input_data["obs"][idxs][: int(0.7 * num_bc)],
        input_data["act"][idxs][: int(0.7 * num_bc)],
    )
    safe_buffer.fill_buffer(input_data["obs"], input_data["act"])
    held_out_data = {
        "obs": input_data["obs"][idxs][int(0.7 * num_bc) :],
        "act": input_data["act"][idxs][int(0.7 * num_bc) :],
    }
    ...
    def compute_loss_safe(data):
        ...
        targets = (
            torch.lt(torch.sum((a - a_pred) ** 2, dim=1), safety_threshold)
            .float()
            .detach()
        )
    ...
    for i in range(replay_buffer.size):
        a_pred = ac.act(
            torch.as_tensor(safe_buffer.obs_buf[i], dtype=torch.float32, device=device)
        )
    ...
    print(
        "{}% of BC data points are not safe with threshold {}".format(
            num_unsafe * 100 / replay_buffer.size, tau_sup
        )
    )
    ...
    for t in range(iters):
        ...
        while i < obs_per_iter:
            ...
            if robosuite:
                robosuite_cfg["INPUT_DEVICE"].start_control()
            obs, act, rew, done, sup, safety, disc = (
                [o],
                [],
                [],
                [],
